...
class MainPresenter(private val mainView: MainView, private val context: Context) {
    ...
    fun searchCharacters() {
        ...
        mainInteractor.searchByName(name, 0)
    }

    fun searchCharactersNextPage(offset: Int) {
        val name = (context as Activity).findViewById<EditText>(R.id.characterNameEditText).text.toString()
        context.findViewById<ProgressBar>(R.id.charactersProgressBar).visibility = View.VISIBLE
        mainInteractor.searchByNameNextPage(name, offset)
    }
    ...
    fun onNextPageCharactersSearchSuccess(characters: List<MarvelCharacter>, offset: Int, count: Int, total: Int) {
        mainView.onNextPageCharactersSearchSuccess(characters, offset, count, total)
    }
}